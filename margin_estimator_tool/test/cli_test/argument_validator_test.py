    ...
    def test_validate_dir_exists(self):
        ...
        with patch("os.path.exists", return_value=True):
            try:
                ...
                pytest.fail(
                    "validate_dir raised BadParameter unexpectedly for existing directory"
                )
...
class TestMarginCalculatorValidator:
    ...
    def test_validate_invalid_csv_file(self):
        ...
        ):  # csv_file not exists, export_dir exists
            with pytest.raises(click.BadParameter) as excinfo:
                self.validator.validate(
    ...
    def test_validate_invalid_export_dir(self):
        ...
        ):  # csv_file exists, export_dir not exists
            with pytest.raises(click.BadParameter) as excinfo:
                self.validator.validate(
                    csv_file="/path/to/file.csv",
                    date_from="20250101",
...
class TestGetProductsValidator:
    ...
    def test_validate_all_valid(self):
        ...
        with patch("os.path.exists", return_value=True):
            try:
                self.validator.validate(date="20250101", export_dir="/path/to/export")
            except click.BadParameter:
                pytest.fail("Validation failed with valid parameters")
    ...
    def test_validate_none_parameters(self):
        ...
        try:
            self.validator.validate(date=None, export_dir=None)
        except Exception:
    ...
    def test_validate_invalid_date(self):
        ...
        with pytest.raises(click.BadParameter) as excinfo:
            self.validator.validate(date="invalid", export_dir=None)
        assert "Date format" in str(excinfo.value)
    ...
    def test_validate_invalid_export_dir(self):
        ...
        with patch("os.path.exists", return_value=False):
            with pytest.raises(click.BadParameter) as excinfo:
                self.validator.validate(date=None, export_dir="/nonexistent/export")
            assert "not found" in str(excinfo.value)
...
class TestGetSeriesValidator:
    ...
    def test_validate_all_valid(self):
        ...
        with patch("os.path.exists", return_value=True):
            try:
                self.validator.validate(date="20240101", export_dir="/path/to/export")
            except click.BadParameter:
                pytest.fail("Validation failed with valid parameters")
...
class TestGetSnapshotsValidator:
    ...
    def test_validate_all_valid(self):
        """Test validation with all valid parameters."""
        try:
            self.validator.validate(date_from="20250101", date_to="20250131")
        except click.BadParameter:
            pytest.fail("Validation failed with valid parameters")
    ...
    def test_validate_none_parameters(self):
        """Test validation with None parameters which are optional."""
        try:
            self.validator.validate(date_from=None, date_to=None)
        except Exception:
            pytest.fail("Validation failed with None parameters")
    ...
    def test_validate_invalid_date_from(self):
        """Test validation with invalid date_from."""
        with pytest.raises(click.BadParameter) as excinfo:
            self.validator.validate(date_from="invalid", date_to="20250131")
        assert "Date format" in str(excinfo.value)

    def test_validate_invalid_date_to(self):
        """Test validation with invalid date_to."""
        with pytest.raises(click.BadParameter) as excinfo:
            self.validator.validate(date_from="20250101", date_to="invalid")
        assert "Date format" in str(excinfo.value)
...
class TestEtdPortfolioValidator:
    ...
    def test_validate_only_required_valid(self):
        ...
        with patch("os.path.exists", return_value=True):
            try:
                self.validator.validate(
                    csv_file="/path/to/file.csv", date=None, export_dir=None
                )
            except click.BadParameter:
    ...
    def test_validate_invalid_csv_file(self):
        ...
        with patch("os.path.exists", return_value=False):
            with pytest.raises(click.BadParameter) as excinfo:
                self.validator.validate(
                    csv_file="/nonexistent/file.csv", date=None, export_dir=None
                )
            assert "not found" in str(excinfo.value)
    ...
    def test_validate_invalid_date(self):
        ...
        with patch("os.path.exists", return_value=True):
            with pytest.raises(click.BadParameter) as excinfo:
                self.validator.validate(
                    csv_file="/path/to/file.csv", date="invalid", export_dir=None
                )
            assert "Date format" in str(excinfo.value)
