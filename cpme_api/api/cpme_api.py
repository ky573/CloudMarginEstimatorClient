...
class CpmeApi(BaseApi):
    """
    It is recommended to pass instance of Configuration class for eg. logging
    ...
    For any *_post and *_get function there are four optional parameters which has impact only on the individual request. ‘async_req’, ‘verbose’, ‘request_timeout’, ‘api_key’

    :param bool async_req=True: enable asynchronous request and function returns a thread more information about async usage see ..
    :param bool verbose=False: Disable info messages of particular request. It is True as default.
    :param int request_timeout=5: Define timeout interval for one request.

    :param str api_key or x_dbp_apikey: your key, obtain it by registering at [DBG Digital Business Platform](https://console.developer.deutsche-boerse.com/) (required)

    **asynchronous HTTP request**

    .. note::
        >>> thread = api.clearing_currencies_get(api_key, async_req=True)
        >>> result = thread.get()

    """

    def __init__(self, configuration: Configuration = None, set_validation: bool = None):
        """It is recommended to pass instance of Configuration for logging"""
    ...
    def clearing_currencies_get(self, **kwargs):
        ...
        :param int business_date: Business date as of which the result is calculated, in YYYYMMDD format
        :param bool live: Is the snapshot live (a.k.a. intraday)? False for end-of-day.
        :param int live_timestamp: Timestamp as of which the result is calculated, in milliseconds from epoch. Zero means the first live snapshot.
    ...
    def products_get(self, **kwargs):
        ...
        Lists all exchange-traded products, Eurex and ECC. Only `product` and `instrument_type` are returned by default. Remaining attributes (see response) are returned only if specified in `extrafields`.
        This method makes a synchronous HTTP request by default.
    ...
    def series_get(self, **kwargs):
        ...
        List all series of exchange traded product(s). Attributes upto `iid` are returned by default. Remaining attributes (see response) are returned only if specified in `extrafields`.
        This method makes a synchronous HTTP request by default.
    ...
    def snapshots_get(self, **kwargs):
        ...
        :param int business_date_from: Start of a date range, in YYYYMMDD format
        :param int business_date_to: End of a date range, in YYYYMMDD format
    ...
    def estimator_post(self, body, **kwargs):
        ...
        Margin Calculation Portfolio is sent in the request and margin is returned as a response. The request can contain exchange traded derivatives (ETD) portfolio, OTC portfolio or Cash Market portfolio (equities and bonds) or any combination of these assets:
        ...
        ETD portfolio as JSON array `etd_portfolio` instruments are specified either by full key  or by technical `iid`. The technical `iid` takes precedence if both keys are sent. The position always needs `line_no` and `net_ls_balance`.
        The full key for series depends on instrument type, these are the mandatory fields:
            - Future:
                - `product`
                - `contract_date` as YYYYMMDD
                - `version_number` (defaults to 0)
            - Option:
                - `product`
                - `contract_date` as YYYYMMDD
                - `call_put_flag`
        ...
        - OTC reports CB202, CB207.
            - See Eurex OTC member documentation for CB202 and CB207 reports. Only positions from one specified account or risk netting unit (ARNU) will be evaluated, see the attributes of `otc_cb202` structure below. We recommend using gzip compression in the request, see above.

        - OTC FpML.
            - See the public definition for FpML format. It is submitted as `otc_fpml`. The `party` attribute is mandatory, see below. We recommend using gzip compression in the request, see above.

        - Repo JSON.
            - `portfolio_components/repo_json` defines special Repo (single-ISIN Repo) positions. For attribute details see the model below.
    ...
    def otc_sensitivities_post(self, body, **kwargs):
        ...
        Return a table of DV01 (delta) sensitivities of OTC portfolio, per curve and maturity bucket. The sensitivity table is accepted as `otc_sensitivities` input for POST /estimator request and can be used instead of the whole OTC portfolio for faster margin computation.
        This method makes a synchronous HTTP request by default.
    ...
    def otc_trade_details_post(self, body, **kwargs):
        ...
        Return a short human-readable description of OTC trade, e.g. to display on a GUI. The description is not complete, i.e. it is not sufficient to evaluate the trade.
        This method makes a synchronous HTTP request by default.
